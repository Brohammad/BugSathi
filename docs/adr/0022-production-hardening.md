...
2. **Rate limiting** — in-process token bucket per client IP (`golang.org/x/time/rate`). Stricter bucket for `/v1/auth/*`. Returns `429` + `Retry-After`. Disabled when `RATE_LIMIT_RPS=0`.
