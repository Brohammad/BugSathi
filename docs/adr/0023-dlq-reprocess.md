...
1. **Max attempts** — `KAFKA_RETRY_MAX_ATTEMPTS` (default 5). Track attempts in-process by `topic/partition/offset`. After the cap, publish a dead-letter envelope to `{source}.dlq` and **commit** the source offset so the consumer advances.
