...
func (s *Store) MarkProcessing(ctx context.Context, id uuid.UUID, at time.Time) error {
	ct, err := s.pool.Exec(ctx, `
		UPDATE recordings SET status = 'PROCESSING', updated_at = $2
		WHERE id = $1 AND status IN ('UPLOADED', 'FAILED', 'PROCESSING')`, id, at)
	if err != nil {
		return err
	}
...
func (s *Store) MarkFailed(ctx context.Context, id uuid.UUID, at time.Time) error {
	_, err := s.pool.Exec(ctx, `
		UPDATE recordings SET status = 'FAILED', updated_at = $2
		WHERE id = $1`, id, at)
	return err
}
...
func (s *Store) FinalizeReady(
	...
	if _, err := tx.Exec(ctx, `
		UPDATE recordings SET status = 'READY', updated_at = $2
